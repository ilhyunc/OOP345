...
namespace sdds
{
	...
	ProcessData::ProcessData(const std::string& filename, int n_threads) {
		...
		ifstream f(filename, ios::binary);//2진법 함수 사용. 0과1만 사용
		...
		f.read(reinterpret_cast<char*>(&total_items), sizeof(int));
	...
	{
		vector<thread> avgThreads;//thread사용시, ~.join()으로 끝나야함. ##main thread와의 끝나는 타이밍을 맞추기 위해.
		for (int i = 0; i < num_threads; ++i)
		...
		{ thread.join(); }//main thread에 기다려 달라고 하는거.
		...
		ofstream of(target_file, ios::binary);
		if (of.is_open())
		{
			of.write(reinterpret_cast<char*>(&total_items), sizeof(int));
			of.write(reinterpret_cast<char*>(data), total_items * sizeof(int));
		}
		else
		{
